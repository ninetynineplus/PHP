...
class Backend_api extends CI_Controller {
    ...
    public function ajax_filter_customers() {
    	try {
            ...
            foreach($customers as &$customer) {
				//hilangkan yang ga perlu
				unset($customer["password"]);
				unset($customer["id_users"]);
				unset($customer["salt"]);
                ...
                foreach($appointments as &$appointment) {
					...
					unset($appointment["provider"]["settings"]);
                }
    ...
    public function ajax_save_service_item() {
        try {
            ...
            $REQUIRED_PRIV = (!isset($item['id']))
                    ? $this->privileges[PRIV_INVENTORY]['add']
                    : $this->privileges[PRIV_INVENTORY]['edit'];
            if ($REQUIRED_PRIV == FALSE) {
                throw new Exception('You do not have the required privileges for this task.');
            }

            $item_id = $this->inventory_model->save_item($item);
            echo json_encode(array(
                'status' => AJAX_SUCCESS,
                'id' => $item['id']
            ));
    ...
    public function ajax_save_service_item_usage() {
        try {
            $this->load->model('inventory_model');
            $item = json_decode($_POST['item'], true);
            ...
            $item_id = $this->inventory_model->save_item_usage($item);
            echo json_encode(array(
                'status' => AJAX_SUCCESS,
                'id' => $item['id']
            ));
   ...
   /**
    ...
    public function ajax_delete_service_item() {
        try {
            if ($this->privileges[PRIV_INVENTORY]['delete'] == FALSE) {
                throw new Exception('You do not have the required privileges for this task.');
            }

            $this->load->model('inventory_model');
            $result = $this->inventory_model->delete_item($_POST['item_id']);
            echo ($result) ? json_encode(AJAX_SUCCESS) : json_encode(AJAX_FAILURE);
        } catch(Exception $exc) {
            echo json_encode(array(
                'exceptions' => array(exceptionToJavaScript($exc))
   ...
   /**
     ...
     * @param numeric $_POST['category_id'] Record id to be deleted.
     */
    public function ajax_delete_service_item_usage() {
        try {
            if ($this->privileges[PRIV_INVENTORY]['delete'] == FALSE) {
                throw new Exception('You do not have the required privileges for this task.');
            }
            ...
            $this->load->model('inventory_model');
            $result = $this->inventory_model->delete_item_usage($_POST['usage_id']);
            echo ($result) ? json_encode(AJAX_SUCCESS) : json_encode(AJAX_FAILURE);
        } catch(Exception $exc) {
            echo json_encode(array(
                'exceptions' => array(exceptionToJavaScript($exc))
     ...
     * @return array Returns a json encoded array back to client with the category records.
     */
    public function ajax_filter_service_items() {
        try {
            if ($this->privileges[PRIV_INVENTORY]['view'] == FALSE) {
                throw new Exception('You do not have the required privileges for this task.');
            }
            ...
            $key = $this->db->escape_str($_POST['key']);
            $where = '(name LIKE "%' . $key . '%" OR description LIKE "%' . $key . '%")';
            $items = $this->inventory_model->get_all_items($where);
     ...
     * @return array Returns a json encoded array back to client with the category records.
     */
    public function ajax_filter_service_item_usage() {
        try {
            if ($this->privileges[PRIV_INVENTORY]['view'] == FALSE) {
                throw new Exception('You do not have the required privileges for this task.');
            }
            ...
            $this->load->model('inventory_model');
            $key = $this->db->escape_str($_POST['key']);
            $items = $this->inventory_model->get_all_usage($key);
            echo json_encode($items);
        } catch(Exception $exc) {
            echo json_encode(array(
                'exceptions' => array(exceptionToJavaScript($exc))
    ...
    public function ajax_save_booking() {
        try {
            ...
            $bookid =$this->appointments_model->addBooking($data);
            echo json_encode(array(
                'status' => AJAX_SUCCESS,
                'id' => $bookid
            ));
