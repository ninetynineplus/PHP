...
class Customers_Model extends CI_Model {
    ...
    protected function _insert($customer) {
		...
		if (!isset($settings['password']) || !isset($settings['username'])) {
			throw new Exception('Please input password.');
        }

		if(strlen($settings['username']) > 16 || strlen($settings['username']) < 4)
		{
			throw new Exception('Username must be 4 - 16 character');
		}
		
        // Validate password
        if (isset($settings['password'])) {
            if (strlen($settings['password']) < MIN_PASSWORD_LENGTH) {
		...
		$settings['id_users'] = $insertid;	
		
		$where = "ea_user_settings.username = '".$this->db->escape_str($settings["username"])."'";
		$tmp = $this->get_user($where);
		if($tmp) throw new Exception('Username already exists');	
    ...
    protected function _update($customer) {
		...
		$settings['id_users'] = (int)$customer['id'];
		
		$where = "ea_users.id = '".$customer["id"]."'";
		$rec = $this->get_user($where);
		
		if(!$rec) throw new Exception('User not found.');
		
		//klo ganti username
		if($rec["username"] != $settings["username"])
		{
			$where = "ea_user_settings.username = '".$this->db->escape_str($settings["username"])."'";
			$tmp = $this->get_user($where);
			if($tmp) throw new Exception('Username already exists');
        ...
        if (isset($settings['password'])) {
            $salt = $this->db->get_where('ea_user_settings', array('id_users' => $customer['id']))->row()->salt;
			if(!$salt || $salt == '')
			{
				$salt = generate_salt();
				$settings['salt'] = $salt;
	...
	public function get_user($where_clause) {
        ...
        if (!$where_clause) {
            throw new Exception('Invalid argument provided as where clause');
        }

        return $this->db
			->select('ea_users.*, ea_user_settings.*')
			->from('ea_users')
