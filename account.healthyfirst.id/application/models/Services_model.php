...
class Services_Model extends CI_Model {
    ...
    protected function _insert($service) {
		$data = $service;
		$prices = json_decode($service["packprice"]);
		unset($data["packprice"]);
		
        if (!$this->db->insert('ea_services', $data)) {
            throw new Exception('Could not insert service record.');
        }
		
		$this->updateServicePrice($this->db->insert_id(), $prices);
    ...
    protected function _update($service) {
		$data = $service;
		$prices = $service["packprice"];
		unset($data["packprice"]);	
		
		$this->db->where('id', $data['id']);
		if (!$this->db->update('ea_services', $data)) {
		   throw new Exception('Could not update service record');
		}
		
		$this->updateServicePrice($data['id'], $prices);
    }
	
	private function updateServicePrice($serviceid, $prices)
	{
		if(count($prices) > 0)
		{
			$sql = "DELETE FROM ea_services_pack_prices where serviceid = ".(int)$serviceid;
			$this->db->query($sql);
			...
			foreach($prices as $k => $v)
			{
				...
				$this->db->insert('ea_services_pack_prices', $data);
			}			
		}		
	}
    ...
    public function get_batch($where_clause = NULL) {
		...
		$res = $this->db->get('ea_services')->result_array();
		
		$pack_price = $this->db->get('ea_services_pack_prices')->result_array();
		
		$price = array();
		if(count($pack_price) > 0 )
		{
			foreach($pack_price as $k=>$v)
			{
				$obj = new stdClass();
				$obj->serviceid = $v["serviceid"];
				$obj->duration = $v["duration"];
				$obj->price = $v["price"];
				$price[$v["serviceid"]][] = $obj;
			}
		}
		
		for($i = 0;$i<count($res); $i++)
		{
			if(isset($price[$res[$i]["id"]]))
			{
				$res[$i]["packprice"] = $price[$res[$i]["id"]];
			}
		}
	...
	{
		try{
			$query = "select * from ea_service_categories";
	...
	public function getServices($category, &$result)
	{
		try{
			$query = "select * from ea_services where id_service_categories = ".(int)$category;
			$result = $this->db->query($query)->result_array();
		} catch(Exception $exc) {
            return false;
        }
		...
		{
			if(isset($price[$result[$i]["id"]]))
			{
				$result[$i]["packprice"] = $price[$result[$i]["id"]];
			}
		}
	...
	public function getService($id, &$result)
	{
		try{
			$query = "select * from ea_services where id = ".(int)$id;
			$result = $this->db->query($query)->result_array();
			return true;
		} catch(Exception $exc) {
            return false;
