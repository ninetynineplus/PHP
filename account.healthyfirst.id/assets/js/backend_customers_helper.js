...
(function() {
    ...
    CustomersHelper.prototype.display = function(customer) {
        ...
        $.each(customer.appointments, function(index, appointment) {
            var start = '';
			if(appointment.start_datetime > '1970-01-01')
			{
				start = GeneralFunctions.formatDate(Date.parse(appointment.start_datetime), GlobalVariables.dateFormat, true);
			}
			
			var ProvName = 'Unassigned';
			if(appointment.provider.first_name != null)
			{
				ProvName = appointment.provider.first_name + ' ' + appointment.provider.last_name;
			}
